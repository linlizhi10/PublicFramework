...
- (void)viewDidLoad
{
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, SC_APP_SIZE.width + CAMERA_TOPVIEW_HEIGHT+CAMERA_HEIGHT_AUTO);
    }
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, self.view.frame.size.width, self.view.frame.size.width+CAMERA_HEIGHT_AUTO)];
...
- (void)addbottomContainerView {
    float bottom_add_height;
    if (kIOS7) {
        if (!iPhone5) {
            bottom_add_height = 24;
        }
    }
    CGFloat bottomY = _captureManager.previewLayer.frame.origin.y + _captureManager.previewLayer.frame.size.height;
    CGRect bottomFrame = CGRectMake(0, bottomY, SC_APP_SIZE.width, SC_APP_SIZE.height - bottomY + bottom_add_height);
...
- (void)addCameraMenuView {
    ...
    [self buildButton:CGRectMake((SC_APP_SIZE.width - cameraBtnLength) / 2, (_bottomContainerView.frame.size.height - downH - cameraBtnLength) / 2 - sep_height + CAMERA_HEIGHT_AUTO/4, cameraBtnLength, cameraBtnLength)
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        upFrame.size.height = (toShow ? SC_APP_SIZE.width / 2 + CAMERA_TOPVIEW_HEIGHT + CAMERA_HEIGHT_AUTO : 0);
        _doneCameraUpView.frame = upFrame;
        
        CGRect downFrame = _doneCameraDownView.frame;
        downFrame.origin.y = (toShow ? SC_APP_SIZE.width / 2 + CAMERA_TOPVIEW_HEIGHT + CAMERA_HEIGHT_AUTO : _bottomContainerView.frame.origin.y);
...
- (void)takePictureBtnPressed:(UIButton*)sender {
    ...
    actiView.center = CGPointMake(self.view.center.x, self.view.center.y - CAMERA_TOPVIEW_HEIGHT - CAMERA_HEIGHT_AUTO);
