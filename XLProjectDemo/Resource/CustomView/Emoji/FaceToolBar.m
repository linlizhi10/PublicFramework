...
-(id)initWithFrame:(CGRect)frame superView:(UIView *)superView{
    ...
    if (self) {
        // Initialization code
        allPageNum = [[Emoji allEmoji] count]/numOfPage>0?[[Emoji allEmoji] count]/numOfPage+1:[[Emoji allEmoji] count]/numOfPage;
...
//            NSInteger pageNum = [[Emoji allEmoji] count]/35+1;
            for (int i=0; i<allPageNum; i++) {
                ...
                [fview loadFacialView:i size:CGSizeMake(49, 43)];
                fview.delegate=self;
        ...
        scrollView.contentSize=CGSizeMake(320*allPageNum, keyboardHeight);
...
//        pageControl.currentPageIndicatorTintColor=RGBACOLOR(132, 104, 77, 1);
        ...
        pageControl.numberOfPages = allPageNum;//指定页面个数
...
-(void)selectedFacialView:(NSString*)str
{
    ...
    if ([str isEqualToString:@"删除"]) {
        if (textView.text.length>0) {
            if (textView.text.length>1 && [[Emoji allEmoji] containsObject:[textView.text substringFromIndex:textView.text.length-2]]) {
