...
#define k_color_pre     RGBCOLOR(3,186,165)//预览按钮的颜色
#define k_color_finish  RGBCOLOR(3,186,165)//完成按钮的颜色
#define k_color_navTop  RGBCOLOR(3,186,165)//导航栏顶部栏颜色
...
- (IBAction)previewImage:(UIButton *)sender;
...
- (void)viewDidLoad
{
    ...
    self.preBtn.backgroundColor = k_color_pre;
    self.finishBtn.backgroundColor = k_color_finish;
...
- (void)initNav{
    [self addRightNavigationTitleButton:@"拍照"];
}
...
- (void)setupGroupPickerview
{
    ...
    self.navigationTop.backgroundColor = k_color_navTop;
...
- (void)setupCollectionView
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, self.navigationTop.frame.origin.y+self.navigationTop.frame.size.height, 320, self.view.bounds.size.height - 64 -48) collectionViewLayout:layout];
...
- (void)initPhotoBrowser{
    ...
    for (int i=0; i<[selectedPath count]; i++) {
        //        IMG *img = [imgs objectAtIndex:i];
        // Photos
//        photo = [MWPhoto photoWithURL:[NSURL fileURLWithPath:[FileHelper getImageFilePathInDocumentDirectory:[imgs objectAtIndex:i] checkExist:NO]]];
        
        NSIndexPath *idxPath = [selectedPath objectAtIndex:i];
        
//        ALAsset *representation = [self.assets objectAtIndex:idxPath.row];
//        UIImage *img = [UIImage imageWithCGImage:representation.defaultRepresentation.fullResolutionImage
//                                           scale:representation.defaultRepresentation.scale
//                                     orientation:(UIImageOrientation)representation.
//                        defaultRepresentation.orientation];
//        photo = [MWPhoto photoWithImage:img];
        
        
        ALAsset *result = [self.assets objectAtIndex:idxPath.row];
